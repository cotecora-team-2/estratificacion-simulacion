...
# configuraciones
estratif_conf <- crossing(num_gpos_marginacion = c(1,2),
         num_gpos_ln = c(1,2), 
...
conteo_2018_estados <- conteo_2018_estados %>% 
  ...
  mutate(comp_votos = ifelse(is.na(comp_votos), mean(comp_votos, na.rm = TRUE),
                             comp_votos)) %>% 
  select(CLAVE_CASILLA:ID_CASILLA, AMLO:JAMK, state_abbr, .fittedPC1,
         LISTA_NOMINAL_CASILLA, comp_votos)
...
marco_conf <- map_df(estratif_conf, function(x){
  ...
  mutate(indice_votos = ntile(comp_votos, num_gpos_marginacion)) %>%
...
```{r prep-muestras}
set.seed(8834)
generar_muestras <- TRUE
if(generar_muestras){
  frac_muestreo <- c(0.04, 0.10)
  muestras_sim <- map_df(frac_muestreo, function(frac_m){
    map_df(1:80, ~ selec_muestra(.x, frac_m)) %>% 
    select(CLAVE_CASILLA, NOMBRE_ESTADO, ID_CASILLA, estratificacion_num, 
           estrato_df, time, frac_m, id)
  })
  saveRDS(muestras_sim, "../datos/muestras_sim_estratos_votos_80.rds")
} else {
  muestras_sim <- readr::read_rds("../datos/muestras_sim_estratos_votos_50.rds") 
}
```
...
estimaciones_sin_censura <-
  ...
  future_map_dfr(function(data){
    ...
    rat_estimate <- ratio_estimation(data, stratum = estrato_df,
                            ...
                            parties = any_of(partidos), std_errors = TRUE, B = 20) %>% 
...
readr::write_rds(estimaciones_sin_censura, "../datos/estimaciones_sin_censura_less_50.rds")
```
...
```{r}
ggplot(resumen_cens %>% filter(party=="RAC", estratificacion_num %in% c(1:8)), aes(x = frac_m, y = cobertura, 
...
```{r}
ggplot(resumen_cens %>% filter(estratificacion_num %in% 1:8), 
